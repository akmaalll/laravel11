<?php

namespace App\Http\Controllers\App;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class TitleAnalysisController extends Controller
{
    /**
     * Check title similarity using cosine similarity algorithm
     */
    public function checkTitleSimilarity(Request $request)
    {
        try {
            // Validasi input
            $request->validate([
                'judul' => 'required|string|min:10|max:500'
            ]);

            $inputTitle = $request->input('judul');

            // Ambil semua judul dari database
            $existingTitles = DB::table('mst_juduls')
                ->select('id', 'judul', 'created_at')
                ->get();

            if ($existingTitles->isEmpty()) {
                return response()->json([
                    'success' => true,
                    'similarity' => 0,
                    'message' => 'Tidak ada judul pembanding di database',
                    'similar_titles' => []
                ]);
            }

            // Hitung similarity untuk setiap judul
            $similarities = [];
            foreach ($existingTitles as $existingTitle) {
                $similarity = $this->calculateCosineSimilarity($inputTitle, $existingTitle->judul);

                if ($similarity > 0.1) { // Hanya simpan yang similarity > 10%
                    $similarities[] = [
                        'id' => $existingTitle->id,
                        'judul' => $existingTitle->judul,
                        'similarity' => $similarity,
                        'created_at' => $existingTitle->created_at
                    ];
                }
            }

            // Urutkan berdasarkan similarity tertinggi
            usort($similarities, function ($a, $b) {
                return $b['similarity'] <=> $a['similarity'];
            });

            // Ambil similarity tertinggi
            $maxSimilarity = !empty($similarities) ? $similarities[0]['similarity'] : 0;

            // Ambil 5 judul teratas yang mirip untuk ditampilkan
            $topSimilarTitles = array_slice($similarities, 0, 5);

            return response()->json([
                'success' => true,
                'algorithm' => 'cosine_similarity',
                'similarity' => $maxSimilarity,
                'similar_titles' => $topSimilarTitles,
                'total_compared' => $existingTitles->count(),
                'message' => $this->getSimilarityMessage($maxSimilarity)
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'success' => false,
                'message' => 'Data tidak valid: ' . implode(', ', $e->validator->errors()->all())
            ], 422);
        } catch (\Exception $e) {
            Log::error('Error in checkTitleSimilarity: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
        }
    }

    /**
     * Classify title topic using SVM-like algorithm
     */
    public function classifyTitleTopic(Request $request)
    {
        try {
            // Validasi input
            $request->validate([
                'judul' => 'required|string|min:10|max:500'
            ]);

            $inputTitle = $request->input('judul');

            // Preprocessing input title
            $preprocessedTitle = $this->preprocessText($inputTitle);

            // Get training data from database
            $trainingData = DB::table('mst_juduls')
                ->select('judul', 'topik')
                ->get();

            if ($trainingData->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Tidak ada data training untuk klasifikasi'
                ], 404);
            }

            // Prepare documents for TF-IDF calculation
            $documents = [];
            $trainingSet = [];

            foreach ($trainingData as $data) {
                $preprocessedDoc = $this->preprocessText($data->judul);
                $documents[] = $preprocessedDoc;
                $trainingSet[] = [
                    'text' => $preprocessedDoc,
                    'topic' => $data->topik
                ];
            }

            // Calculate TF-IDF for input title
            $inputTfidf = $this->calculateTFIDF($preprocessedTitle, $documents);

            // SVM-like classification using cosine similarity
            $maxSimilarity = -1;
            $predictedTopic = null;

            foreach ($trainingSet as $trainDoc) {
                $docTfidf = $this->calculateTFIDF($trainDoc['text'], $documents);

                // Calculate cosine similarity
                $similarity = $this->calculateTFIDFCosineSimilarity($inputTfidf, $docTfidf);

                if ($similarity > $maxSimilarity) {
                    $maxSimilarity = $similarity;
                    $predictedTopic = $trainDoc['topic'];
                }
            }

            return response()->json([
                'success' => true,
                'algorithm' => 'svm_classification',
                'input_title' => $inputTitle,
                'predicted_topic' => $predictedTopic,
                'similarity_score' => $maxSimilarity,
                'message' => $this->getClassificationMessage($maxSimilarity)
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'success' => false,
                'message' => 'Data tidak valid: ' . implode(', ', $e->validator->errors()->all())
            ], 422);
        } catch (\Exception $e) {
            Log::error('Error in classifyTitleTopic: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
        }
    }

    /**
     * Combined analysis using both algorithms
     */
    public function analyzeTitle(Request $request)
    {
        try {
            // Validasi input
            $request->validate([
                'judul' => 'required|string|min:10|max:500'
            ]);

            // Get similarity analysis
            $similarityRequest = new Request(['judul' => $request->input('judul')]);
            $similarityResponse = $this->checkTitleSimilarity($similarityRequest);
            $similarityData = $similarityResponse->getData(true);

            // Get classification analysis
            $classificationResponse = $this->classifyTitleTopic($request);
            $classificationData = $classificationResponse->getData(true);

            return response()->json([
                'success' => true,
                'input_title' => $request->input('judul'),
                'similarity_analysis' => $similarityData,
                'classification_analysis' => $classificationData,
                'recommendation' => $this->getRecommendation($similarityData, $classificationData)
            ]);
        } catch (\Exception $e) {
            Log::error('Error in analyzeTitle: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
        }
    }

    /**
     * Calculate cosine similarity between two texts
     */
    private function calculateCosineSimilarity($text1, $text2)
    {
        // Preprocessing teks
        $text1 = $this->preprocessText($text1);
        $text2 = $this->preprocessText($text2);

        // Tokenisasi
        $tokens1 = $this->tokenize($text1);
        $tokens2 = $this->tokenize($text2);

        // Buat vocabulary dari kedua teks
        $vocabulary = array_unique(array_merge($tokens1, $tokens2));

        // Hitung term frequency untuk setiap teks
        $vector1 = $this->getTermFrequencyVector($tokens1, $vocabulary);
        $vector2 = $this->getTermFrequencyVector($tokens2, $vocabulary);

        // Hitung cosine similarity
        return $this->cosineSimilarity($vector1, $vector2);
    }

    /**
     * Calculate TF-IDF for a text given documents collection
     */
    private function calculateTFIDF($text, $documents)
    {
        $words = explode(' ', $text);
        $tf = array_count_values($words);
        $tfidf = [];

        $N = count($documents); // Total number of documents

        foreach ($tf as $word => $freq) {
            // Calculate document frequency (df)
            $df = 0;
            foreach ($documents as $doc) {
                if (strpos($doc, $word) !== false) {
                    $df++;
                }
            }

            // Calculate IDF
            $idf = log($N / ($df + 1));

            // Calculate TF-IDF
            $tfidf[$word] = $freq * $idf;
        }

        return $tfidf;
    }

    /**
     * Calculate cosine similarity between two TF-IDF vectors
     */
    private function calculateTFIDFCosineSimilarity($tfidf1, $tfidf2)
    {
        $dotProduct = 0;
        $magnitude1 = 0;
        $magnitude2 = 0;

        // Get all unique words from both vectors
        $allWords = array_unique(array_merge(array_keys($tfidf1), array_keys($tfidf2)));

        foreach ($allWords as $word) {
            $value1 = $tfidf1[$word] ?? 0;
            $value2 = $tfidf2[$word] ?? 0;

            $dotProduct += $value1 * $value2;
            $magnitude1 += $value1 * $value1;
            $magnitude2 += $value2 * $value2;
        }

        if ($magnitude1 > 0 && $magnitude2 > 0) {
            return $dotProduct / (sqrt($magnitude1) * sqrt($magnitude2));
        }

        return 0;
    }

    /**
     * Preprocess text (lowercase, remove punctuation, etc.)
     */
    private function preprocessText($text)
    {
        // Convert to lowercase
        $text = strtolower($text);

        // Remove punctuation and special characters
        $text = preg_replace('/[^\p{L}\p{N}\s]/u', ' ', $text);

        // Remove extra whitespace
        $text = preg_replace('/\s+/', ' ', $text);

        return trim($text);
    }

    /**
     * Tokenize text into words
     */
    private function tokenize($text)
    {
        // Split by whitespace and remove empty tokens
        $tokens = array_filter(explode(' ', $text), function ($token) {
            return !empty(trim($token));
        });

        // Remove stopwords
        $stopwords = [
            'yang',
            'dan',
            'di',
            'ke',
            'dari',
            'untuk',
            'dengan',
            'pada',
            'dalam',
            'oleh',
            'sebagai',
            'adalah',
            'akan',
            'telah',
            'dapat',
            'atau',
            'juga',
            'tidak',
            'ada',
            'ini',
            'itu',
            'nya',
            'an',
            'al',
            'el',
            'la',
            'le',
            'de',
            'du',
            'the',
            'of',
            'and',
            'or',
            'in',
            'on',
            'at',
            'by',
            'for',
            'with',
            'to',
            'from',
            'as',
            'is',
            'are',
            'was',
            'were',
            'be',
            'been',
            'have',
            'has',
            'had',
            'do',
            'does',
            'did',
            'will',
            'would',
            'could',
            'should',
            'may',
            'might',
            'can',
            'must',
            'shall'
        ];

        return array_filter($tokens, function ($token) use ($stopwords) {
            return !in_array($token, $stopwords) && strlen($token) > 2;
        });
    }

    /**
     * Get term frequency vector for given tokens and vocabulary
     */
    private function getTermFrequencyVector($tokens, $vocabulary)
    {
        $vector = [];
        $tokenCounts = array_count_values($tokens);

        foreach ($vocabulary as $term) {
            $vector[] = isset($tokenCounts[$term]) ? $tokenCounts[$term] : 0;
        }

        return $vector;
    }

    /**
     * Calculate cosine similarity between two vectors
     */
    private function cosineSimilarity($vector1, $vector2)
    {
        // Hitung dot product
        $dotProduct = 0;
        for ($i = 0; $i < count($vector1); $i++) {
            $dotProduct += $vector1[$i] * $vector2[$i];
        }

        // Hitung magnitude untuk vector1
        $magnitude1 = sqrt(array_sum(array_map(function ($x) {
            return $x * $x;
        }, $vector1)));

        // Hitung magnitude untuk vector2
        $magnitude2 = sqrt(array_sum(array_map(function ($x) {
            return $x * $x;
        }, $vector2)));

        // Avoid division by zero
        if ($magnitude1 == 0 || $magnitude2 == 0) {
            return 0;
        }

        return $dotProduct / ($magnitude1 * $magnitude2);
    }

    /**
     * Validate topic with program study
     */
    private function validateTopicWithProdi($predictedTopic, $kodeJurusan)
    {
        if (!$predictedTopic) {
            return [
                'is_valid' => false,
                'message' => 'Hasil Klasifikasi Tidak Terdeteksi'
            ];
        }

        $topicExists = DB::table('mst_topiks')
            ->where('id_prodi', $kodeJurusan)
            ->where('nama_topik', $predictedTopic)
            ->exists();
        dd($predictedTopic);

        if ($topicExists) {
            return [
                'is_valid' => true,
                'message' => 'Topik sesuai dengan program studi'
            ];
        } else {
            $prodi = DB::table('mst_prodis')
                ->where('kode', $kodeJurusan)
                ->value('nama');

            return [
                'is_valid' => false,
                'message' => "Hasil Klasifikasi Judul Anda Tidak Sesuai Dengan Topik pada Program Studi {$prodi}"
            ];
        }
    }

    /**
     * Get similarity message based on percentage
     */
    private function getSimilarityMessage($similarity)
    {
        $percentage = $similarity * 100;

        if ($percentage < 30) {
            return 'Tingkat kesamaan rendah. Judul dapat digunakan.';
        } elseif ($percentage < 70) {
            return 'Tingkat kesamaan sedang. Disarankan untuk memodifikasi judul.';
        } else {
            return 'Tingkat kesamaan tinggi. Judul tidak dapat digunakan.';
        }
    }

    /**
     * Get classification message
     */
    private function getClassificationMessage($similarity)
    {
        if ($similarity <= 0.0) {
            return 'Tidak dapat mengklasifikasikan judul';
        }

        return 'Klasifikasi topik berhasil';
    }

    /**
     * Get recommendation based on both analyses
     */
    private function getRecommendation($similarityData, $classificationData)
    {
        $recommendation = [];

        // Similarity recommendation
        if ($similarityData['success']) {
            $similarity = $similarityData['similarity'];
            if ($similarity >= 0.7) {
                $recommendation[] = 'Judul memiliki kesamaan tinggi dengan judul yang sudah ada. Disarankan untuk mengubah judul.';
            } elseif ($similarity >= 0.3) {
                $recommendation[] = 'Judul memiliki kesamaan sedang. Pertimbangkan untuk memodifikasi judul.';
            } else {
                $recommendation[] = 'Judul memiliki kesamaan rendah. Judul dapat digunakan dari segi kesamaan.';
            }
        }

        // Classification recommendation
        if ($classificationData['success'] && isset($classificationData['predicted_topic'])) {
            $recommendation[] = 'Judul terklasifikasi sebagai topik: ' . $classificationData['predicted_topic'];
        }

        return $recommendation;
    }
}
